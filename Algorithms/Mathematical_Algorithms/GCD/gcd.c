...
void gcd_of_numbers(int set_one_size, int* set_one, int set_two_size, int* set_two)
{
	int i =0;
	int intersection_set[MAX_SIZE*2];
	int intersection_set_count =0;
	int gcd =1;
	...
	for(i =0; i<set_two_size; i++)
	{
		...
		{
			intersection_set[intersection_set_count] = set_two[i];
			intersection_set_count++;
		}
	}
	for(i=0; i<intersection_set_count; i++)
	{
		gcd *= intersection_set[i];
	}
	printf("\nGCD of two number : %d\n", gcd);
	return;
}
...
int main(void)
{
	...
	gcd_of_numbers(set_one_size, set_one, set_two_size, set_two);
